...
import { cldVideo, cldPoster, cldPosterTiny } from "@/lib/media";

/* Poster frames are generated by Cloudinary from the video itself
   at `posterAt` seconds — no stored thumbnail files. */
const COMMERCIALS = [
  // ── Lead reel, in order ──
  {
    id: "raindance",
  ...
  {
    id: "bts-behind-scenes",
    title: "Snow Bunny",
    meta: "Brand Collab",
    posterAt: 1,
  },
  {
    id: "olamide-rock-me-gentle",
  ...
  ...c,
  src: cldVideo(`candice/commercials/${c.id}`),
  poster: cldPoster(`candice/commercials/${c.id}`, c.posterAt),
  posterTiny: cldPosterTiny(`candice/commercials/${c.id}`, c.posterAt),
}));
...
function VideoCard({
  ...
  // The <video> element is driven by the isPlaying prop — so when another
  // card takes over (isPlaying → false) this one actually pauses.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      ([entry]) => {
        if (!entry.isIntersecting && isPlaying) onScrollAway();
      },
      { threshold: 0.35 },
    );
    obs.observe(el);
    return () => obs.disconnect();
  }, [isPlaying, onScrollAway]);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundImage: `url(${commercial.posterTiny})`,
...
export default function Commercials() {
  ...
  return (
    <section id="commercials" className="py-20 md:py-32 overflow-hidden px-6 md:px-10">
